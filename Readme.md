# David Godbehere - Portfolio Website

A modern, responsive portfolio website showcasing media design, web development, and coding skills.

![Portfolio Preview](Images/preview.png)

## 🌐 Live Demo

Visit the live site: [https://Ddave82.github.io/portfolio](https://Ddave82.github.io/portfolio)

## ✨ Features

- **Modern Design**: Sleek, dark theme with cyan accents and hexagon pattern background
- **Responsive**: Fully responsive design that works on all devices
- **Interactive**: Smooth animations, scroll effects, and hover interactions
- **Performance Optimized**: Lightweight and fast-loading
- **Accessible**: Semantic HTML and ARIA labels

## 🎨 Design Highlights

- Custom hexagon SVG pattern background
- Animated light sweep effect (every 15 seconds)
- Glassmorphism UI elements with backdrop blur
- Smooth scroll-reveal animations
- Active navigation state tracking
- Back-to-top button for easy navigation

## 🛠️ Technologies Used

- **HTML5**: Semantic markup
- **CSS3**: Custom properties, flexbox, grid, animations
- **Vanilla JavaScript**: No frameworks, pure JS for interactions
- **SVG**: Custom icons and graphics

## 📂 Project Structure
